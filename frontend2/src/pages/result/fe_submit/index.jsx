...
function Result() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // 获取存储的参数
                const formData = JSON.parse(localStorage.getItem('fe_design_params'));
                const token = localStorage.getItem('fe_design_token');
                ...
                setParameters({
                    sequences: [
                        {
                            label: "Input Left Sequence",
                            value: formData.input_left_sequences,
                            type: "plain"
                        },
                        {
                            label: "Input Right Sequence",
                            value: formData.input_right_sequences,
                            type: "plain"
                        }
                    ],
                    settings: [
                        { label: "PAM Type", value: formData.pam_type || "NGG" },
                        { label: "Target Genome", value: formData.target_genome },
                        { label: "Customized PAM", value: formData.customized_pam || "-" },
                        { label: "sgRNA Module", value: formData.sgrna_module || "spacerpam" },
                        { label: "Spacer Length", value: formData.spacer_length || "20" },
                        { label: "Sequence Length", value: formData.seq_length || "3" },
                    ]
                });
                ...
                if (data.TableData && data.TableData.rows && data.TableData.rows.length > 0) {
                    setSelectedRow(data.TableData.rows[0]);
                    setOffTargetData(data.TableData.rows[0].offtarget_json.rows);
                }
            ...
            <div className="tables-container">
                <div className="table-section left">
                    ...
                    <div className="table-content">
                        <Table
                            columns={columns}
                            dataSource={resultData?.TableData?.rows || []}
                            pagination={false}
                            rowSelection={{
                                type: 'radio',
                                selectedRowKeys,
