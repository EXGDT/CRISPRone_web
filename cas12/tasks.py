...
def form2resultjson(fasta_sequence_position, pam, spacerLength, sgRNAModule, name_db, task_path, task_id):
    ...
    cas12a_task_object = result_cas12a_list(
        pamType=pam,
        name_db=name_db,
...
def form2Database(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):
    ...
    cas12a_task_record = result_cas12a_list(task_id=task_id, input_sequence=inputSequence, pam_type=pam, spacer_length=spacerLength, sgRNA_module=sgRNAModule, name_db=name_db, task_status='running')
    ...
    step_time = time.time()
    cas12a_task_record.sgRNA_json = guide_json
...
@shared_task
def cas12a_task_process(task_id, inputSequence, pam, spacerLength, sgRNAModule, name_db):

    if result_cas12a_list.objects.filter(task_id=task_id).exists():
